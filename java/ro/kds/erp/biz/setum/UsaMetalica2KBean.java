 ...
 *
 */
public class UsaMetalica2KBean implements SessionBean, SessionSynchronization {
    ...
    public void setSessionContext(SessionContext ctx) {
        if (logger == null) {
            logger = Log.getLogger("ro.kds.erp.biz.setum.UsamMetalica2KBean");
        }
        logger.log(BasicLevel.DEBUG, "");
        ejbContext = ctx;
    }
	...
	try {
	    ...
	    attribs.add(ah.create("lcorrection", new BigDecimal(um.getLcorrection())));
 	    ...
 	    attribs.add(ah.create("lCurrent", new BigDecimal(um.getLCurrent())));
	    attribs.add(ah.create("kType", new Integer(um.getKType())));
	    ...
	    attribs.add(ah.create("intFoilSec", new Integer(um.getIntFoilSec())));
	    attribs.add(ah.create("ieFoilSec", new Integer(um.getIeFoilSec())));
	    attribs.add(ah.create("extFoilSec", new Integer(um.getExtFoilSec())));
	...
	try {
		...
		if(a.getName().compareTo("subclass") == 0) {
 		...
 		} else if(a.getName().compareTo("lCurrent") == 0) {
		    um.setLCurrent(a.getDecimalValue().doubleValue());
		} else if(a.getName().compareTo("kType") == 0) {
		    um.setKType(a.getIntValue().intValue());
		    ...
		    um.setIntFoilSec(a.getIntValue().intValue());
		} else if(a.getName().compareTo("ieFoilSec") == 0) {
		    um.setIeFoilSec(a.getIntValue().intValue());
		} else if(a.getName().compareTo("extFoilSec") == 0) {
		    um.setExtFoilSec(a.getIntValue().intValue());
	...
	ResponseBean r = new ResponseBean();
	um.setLCurrent(lCurrent.doubleValue());
	return r;
    }
    public ResponseBean updateKType(Integer kType) {
	ResponseBean r = new ResponseBean();
	um.setKType(kType.intValue());
	return r;
    }
    ...
    public ResponseBean updateIntFoilSec(Integer intFoil)  {
	ResponseBean r = new ResponseBean();
	um.setIntFoilSec(intFoil.intValue());
	return r;
    }
    public ResponseBean updateIeFoilSec(Integer ieFoil)  {
	ResponseBean r = new ResponseBean();
	um.setIeFoilSec(ieFoil.intValue());
	return r;
    }
    public ResponseBean updateExtFoilSec(Integer extFoil)  {
	ResponseBean r = new ResponseBean();
	um.setExtFoilSec(extFoil.intValue());
	return r;
    }
	...
	r.addField("lCurrent", um.getLCurrent());
	r.addField("kType", um.getKType());
	r.addField("ieFoil", um.getIeFoil());
	r.addField("intFoil", um.getIntFoil());
	r.addField("extFoil", um.getExtFoil());
	r.addField("ieFoilSec", um.getIeFoilSec());
	r.addField("intFoilSec", um.getIntFoilSec());
	r.addField("extFoilSec", um.getExtFoilSec());
	...
	// validation rules
	if(um.getIntFoil() == 1 && um.getLg() < 1050) {
	    r.setCode(1);
	    r.setMessage("Pentru foaie Lisa Lminim = 1050");
	    r.addRecord();
	    r.addField("ruleid", 1);
	    r.addField("errmsg", "Pentru foaie Lisa Lminim = 1050");
	}
	...
	if(um.getLCurrent() < 700) {
	    r.setCode(5);
	    r.setMessage("Lcurrenta trebuie mimim 700");
	    r.addRecord();
	    r.addField("ruleid", 4);
	    r.addField("errmsg", "Lcurrenta trebuie mimim 700");
	}

	if(um.getIntFoil() == 2 && um.getKType() == 1 && um.getLg() < 1485) {
	    r.setCode(6);
	    r.setMessage("Pentru foaie amprentata Lminim = 1485");
	    r.addRecord();
	    r.addField("ruleid", 5);
	    r.addField("errmsg", "Pentru foaie amprentata Lminim = 1485");
	}

	if(um.getIntFoil() == 2 && um.getHg() < 1740) {
	    r.setCode(7);
	    r.setMessage("Pentru foaie amprentata Hminim = 1740");
	    r.addRecord();
	    r.addField("ruleid", 5);
	    r.addField("errmsg", "Pentru foaie amprentata Hminim = 1740");
	}
	...
	if(um.getIntFoil() == 2 && um.getKType() == 2 && um.getLg() < 1065) {
	    r.setCode(9);
	    r.setMessage("Pentru foaie amprentata Lminim = 1065");
	    r.addRecord();
	    r.addField("ruleid", 5);
	    r.addField("errmsg", "Pentru foaie amprentata Lminim = 1065");
	}
