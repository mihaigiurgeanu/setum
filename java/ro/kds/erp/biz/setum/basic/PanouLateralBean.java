...
public abstract class PanouLateralBean 
    ...
    public void setSessionContext(SessionContext ctx) {
        if (logger == null) {
            logger = Monolog.getMonologFactory().getLogger("ro.kds.erp.biz.setum.basic.PanouLateral");
        }
	...
	logger.log(BasicLevel.DEBUG, "Loading PanouLateral with id = " + loadId);
	createNewFormBean();
	id = loadId;
    ...
    /**
     * Create a new PanouLateralForm and initialize the 
     ...
     * 
     * <code>ro.kds.erp.biz.setum.basic.PanouLateral_calculatedFields</code>
     * 
	...
	Script script = TclFileScript
		.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral_calculatedFields");
	if(script.loaded()) {
	    try {
		script.setVar(FORM_VARNAME, form, 
			      PanouLateralForm.class);
		...
		logger.log(BasicLevel.ERROR, "Error executing rules script for PanouLateral", e);
	    }
	}
	return r;
    }
     ...
     *
     * <code>ro.kds.erp.biz.setum.basic.PanouLateral_validation</code>
     *
	...
	Script script = TclFileScript
		.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral_validation");
	if(script.loaded()) {
	    try {
		script.setVar(FORM_VARNAME, form, 
			      PanouLateralForm.class);
    ...
    public ResponseBean updateLpl(Double lpl) {
        ResponseBean r = new ResponseBean();
	Double oldVal = form.getLpl();
	form.setLpl(lpl);
	r.addRecord();
	r.addField("lpl", lpl); // for number format
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.lpl");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Double.class);
		script.setVar(FORM_VARNAME, form, PanouLateralForm.class);
		script.setVar(RESPONSE_VARNAME, r, ResponseBean.class);
		addFieldsToScript(script);
		script.run();
		getFieldsFromScript(script, r); // add all the changed
	       ...
	       logger.log(BasicLevel.ERROR, "Can not run the script for updating the lpl", e);
           }
        }
	computeCalculatedFields(r);
	return r;
    }
    public ResponseBean updateHpl(Double hpl) {
        ResponseBean r = new ResponseBean();
	Double oldVal = form.getHpl();
	form.setHpl(hpl);
	r.addRecord();
	r.addField("hpl", hpl); // for number format
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.hpl");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Double.class);
		...
		getFieldsFromScript(script, r); // add all the changed
	       ...
	       logger.log(BasicLevel.ERROR, "Can not run the script for updating the hpl", e);
           }
        }
	computeCalculatedFields(r);
	return r;
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.cells");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.deschidere");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.sensDeschidere");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.pozitionareBalamale");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.componenta");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.tipComponenta");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.tipGeam");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.geamSimpluId");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.geamTermopanId");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.tipGrilaj");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.grilajStasId");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.valoareGrilajAtipic");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.tipTabla");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.tablaId");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.sellPrice");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, java.math.BigDecimal.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.entryPrice");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, java.math.BigDecimal.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.price1");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, java.math.BigDecimal.class);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.businessCategory");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
	...
	Script script = TclFileScript.loadScript("ro.kds.erp.biz.setum.basic.PanouLateral.quantity");
	if(script.loaded()) {
	   try {
		script.setVar(LOGIC_VARNAME, this);
		script.setVar(OLDVAL_VARNAME, oldVal, Integer.class);
	...
	r.addField("lpl", form.getLpl());
	r.addField("hpl", form.getHpl());
	...
	try {
	    s.setVar("lpl", form.getLpl(), Double.class);
	} catch (ScriptErrorException e) {
	    logger.log(BasicLevel.WARN, "Can not set the value of field: lpl from the script");
            logger.log(BasicLevel.DEBUG, e);
        }
	try {
	    s.setVar("hpl", form.getHpl(), Double.class);
	} catch (ScriptErrorException e) {
	    logger.log(BasicLevel.WARN, "Can not set the value of field: hpl from the script");
            logger.log(BasicLevel.DEBUG, e);
        }
	try {
	...
	Object field;
	try {
	    field = s.getVar("lpl", Double.class);
	    if(!field.equals(form.getLpl())) {
	        logger.log(BasicLevel.DEBUG, "Field lpl modified by script. Its new value is <<" + (field==null?"null":field.toString()) + ">>");
	        form.setLpl((Double)field);
	        r.addField("lpl", (Double)field);
	    }
	} catch (ScriptErrorException e) {
	    logger.log(BasicLevel.WARN, "Can not get the value of field: lpl from the script");
            logger.log(BasicLevel.DEBUG, e);
        }
	try {
	    field = s.getVar("hpl", Double.class);
	    if(!field.equals(form.getHpl())) {
	        logger.log(BasicLevel.DEBUG, "Field hpl modified by script. Its new value is <<" + (field==null?"null":field.toString()) + ">>");
	        form.setHpl((Double)field);
	        r.addField("hpl", (Double)field);
	    }
	} catch (ScriptErrorException e) {
	    logger.log(BasicLevel.WARN, "Can not get the value of field: hpl from the script");
            logger.log(BasicLevel.DEBUG, e);
        }
	try {
