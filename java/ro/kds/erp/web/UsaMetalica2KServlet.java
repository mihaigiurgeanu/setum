...
import ro.kds.erp.biz.setum.UsaMetalica2KHome;
import ro.kds.erp.biz.setum.UsaMetalica2K;

/**
 * Servlet interfacing the UI layer and business logic.
 *
 *
 * Created: Wed Oct 05 20:01:05 2005
 *
 * @author <a href="mailto:Mihai Giurgeanu@CRIMIRA"></a>
 * @version 1.0
 */
public class UsaMetalica2KServlet extends HttpServlet {
	...
	logger = Log.getLogger("ro.kds.erp.web.UsaMetalica2KServlet");
	...
	if(command == null) {
		...
		UsaMetalica2K b = (UsaMetalica2K)httpServletRequest.getSession().getAttribute("session");
		...
		try {
		    ...
		    } else if(field.compareTo("lCurrent") == 0) {
			r = b.updateLCurrent(new Double(Double.parseDouble(value)));
		    } else if(field.compareTo("kType") == 0) {
			r = b.updateKType(new Integer(Integer.parseInt(value)));
		    ...
		    } else if(field.compareTo("intFoilSec") == 0) {
			r = b.updateIntFoilSec(new Integer(Integer.parseInt(value)));
		    } else if(field.compareTo("ieFoilSec") == 0) {
			r = b.updateIeFoilSec(new Integer(Integer.parseInt(value)));
		    } else if(field.compareTo("extFoilSec") == 0) {
			r = b.updateExtFoilSec(new Integer(Integer.parseInt(value)));
		...
		try {
		    ...
		    UsaMetalica2KHome bh = (UsaMetalica2KHome)PortableRemoteObject.narrow(env.lookup("ejb/UsaMetalica2K"),UsaMetalica2KHome.class);

		    // creates and registers a new session bean
		    UsaMetalica2K b = bh.create();
		    httpServletRequest.getSession().setAttribute("session", b);
		    
		    r = b.loadProduct(new Integer(id));
